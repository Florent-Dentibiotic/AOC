...
const defineNextValue = (sequence, gap) => {
  console.log(sequence[sequence.length - 1], gap)
  return Number(sequence[sequence.length - 1]) + gap
}
...
const recursivelyFindSequence = (sequences, index, gap) => {
  ...
  if (gap || gap === 0) {
    ...
    if (index === 0) return defineNextValue(sequences[index], gap)
    return recursivelyFindSequence(
      sequences,
      index - 1,
      defineNextValue(sequences[index], gap)
    )
  }
  //console.log(sequences, index, gap)
